...
class Ea_Model_Layout extends Ea_Model_Abstract
{
	...
	public function __construct($config=null)
	{
		...
		{
			$config=array('data_model'=>new Ea_Model_Data_Table($config));
		}
		else if($config instanceof Ea_Model_Data_Abstract)
		{
	...
	{
		$this->_columns=array();
		foreach($this->_dataModel->sortColumns() as $column)
		{
	...
	public function getColumnAdapter($name)
	{
		...
		$instance=new $class($name, $this->getMetaData($name), $config);
